...
@section('content')
    ...
    <div class="flex">
        ...
        <div class="flex-1 p-8 ml-60">
            ...
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-zinc-900">Daftar Booking</h1>
            ...
            <div class="grid grid-cols-4 gap-4 mb-6">
                ...
                @foreach($cardData as $card)
                    <div class="rounded-xl border border-zinc-200 px-5 py-4 {{ $card['color'] }}">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                @foreach($statuses as $key => $label)
                        <a href="{{ route('admin.daftar-booking', $key !== 'semua' ? ['status' => $key] : []) }}" class="px-4 py-1.5 rounded-full text-sm font-medium border transition-colors
                               {{ $activeFilter === $key
                    ...
                    <table class="min-w-full table-auto border-collapse text-sm">
                        <thead>
                            <tr class="bg-zinc-50 text-left text-xs font-semibold text-zinc-500 uppercase tracking-wider">
                                <th class="px-4 py-3 border-b border-zinc-200">#</th>
                                <th class="px-4 py-3 border-b border-zinc-200">Pemesan</th>
                                <th class="px-4 py-3 border-b border-zinc-200">Lapangan</th>
                                <th class="px-4 py-3 border-b border-zinc-200">Tanggal</th>
                                <th class="px-4 py-3 border-b border-zinc-200">Jam</th>
                                <th class="px-4 py-3 border-b border-zinc-200">Total Harga</th>
                                <th class="px-4 py-3 border-b border-zinc-200">Status</th>
                                <th class="px-4 py-3 border-b border-zinc-200">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($bookings as $i => $booking)
                                ...
                                <tr class="hover:bg-zinc-50 border-b border-zinc-100 last:border-b-0">
                                    ...
                                    <td class="px-4 py-3">
                                        <p class="font-medium text-zinc-900">{{ $booking->user->name }}</p>
                                        <p class="text-xs text-zinc-400">{{ $booking->user->email }}</p>
                                    </td>

                                    <td class="px-4 py-3">
                                        <p class="font-medium text-zinc-900">{{ $booking->lapangan->nama_lapangan }}</p>
                                        ...
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            @if($booking->status === 'pending')
                                                <form action="{{ route('admin.booking.update', $booking->id) }}" method="POST">
                                                    ...
                                                    <button
                                                        class="px-2.5 py-1 text-xs font-medium text-blue-600 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100">
                                                        Konfirmasi
                                                    </button>
                                                </form>
                                            @endif
                                            ...
                                            @if($booking->status === 'confirmed')
                                                <form action="{{ route('admin.booking.update', $booking->id) }}" method="POST">
                                                    ...
                                                    <button
                                                        class="px-2.5 py-1 text-xs font-medium text-green-600 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100">
                                                        Selesai
                                                    </button>
                                                </form>
                                            @endif
                                            ...
                                            @if(in_array($booking->status, ['pending', 'confirmed']))
                                                <form action="{{ route('admin.booking.update', $booking->id) }}" method="POST"
                                                    ...
                                                    <button
                                                        class="px-2.5 py-1 text-xs font-medium text-red-600 bg-red-50 border border-red-200 rounded-lg hover:bg-red-100">
                                                        Tolak
                                                    </button>
                                                </form>
                                            @endif
